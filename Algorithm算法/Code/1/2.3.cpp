...
int partition(int a[], int l, int r) // basic partition
{
  ...
  while (i < j)
  {
    while (a[t(j)] >= pivot && i < j)
      j--;
    a[t(i)] = a[t(j)];
    while (a[t(i)] <= pivot && i < j)
      i++;
    a[t(j)] = a[t(i)];
  }
  a[t(i)] = pivot;
...
int kth_smallest(int a[], int l, int r, int k)
{
  ...
  // partition the aay (similar to quicksort)
  ...
  // if rank-k element is less, look in left sub-aay
  if (index - l > k - 1)
    return kth_smallest(a, l, index - 1, k);
  else // look in right sub-aay
